...
// ── Phase windows (revised May 2026 — RFI removed, 24-month plan) ─
const PHASE_WINDOWS = [
  { id: "phase1", label: "Vendor RFP & Selection",               start: "2026-07", end: "2026-09" },
  { id: "phase2", label: "LOI + Contract Complete",              start: "2026-10", end: "2027-02" },
  { id: "phase3", label: "Parallel Platform Setup",              start: "2026-11", end: "2027-07" },
  { id: "phase4", label: "Pilot Execution & Learning",           start: "2027-08", end: "2028-04" },
  { id: "phase5", label: "Travel Platform Live Decision Point",  start: "2028-05", end: "2028-06" },
];

// ── Overview phases & milestones (revised May 2026 — RFI removed) ─
const OVERVIEW_PHASES = [
  // Vendor & Platform Readiness
  { laneId:"vendor", id:"rfp",              label:"Vendor RFP",             start:"2026-07", end:"2026-09", color:"#6C63FF", kind:"phase" },
  { laneId:"vendor", id:"downselect",       label:"Down-Select",            start:"2026-08", end:"2026-08", color:"#F7B731", kind:"milestone" },
  { laneId:"vendor", id:"selection",        label:"Final Selection",        start:"2026-09", end:"2026-09", color:"#F7B731", kind:"milestone" },
  { laneId:"vendor", id:"loi",              label:"Letter of Intent",       start:"2026-10", end:"2026-10", color:"#F7B731", kind:"milestone" },
  { laneId:"vendor", id:"contract",         label:"Contract Negotiation",   start:"2026-09", end:"2027-02", color:"#3c82c8", kind:"phase" },
  { laneId:"vendor", id:"contractComplete", label:"Contract Complete",      start:"2027-02", end:"2027-02", color:"#F7B731", kind:"milestone" },
  { laneId:"vendor", id:"setup",            label:"Platform Setup",         start:"2026-11", end:"2027-07", color:"#2EC4B6", kind:"phase" },
  // Pilot Execution
  { laneId:"pilot",  id:"p1",               label:"Pilot 1 — RetailMeNot",                          start:"2027-08", end:"2027-10", color:"#FF6B35", kind:"phase" },
  { laneId:"pilot",  id:"p2",               label:"Pilot 2 — Tech Flagships (ZDNet, PCMag, CNET)",  start:"2027-12", end:"2028-02", color:"#FF4081", kind:"phase" },
  { laneId:"pilot",  id:"p3",               label:"Pilot 3 — Mashable, Spiceworks, Lifehacker",     start:"2028-04", end:"2028-06", color:"#9B59B6", kind:"phase" },
  { laneId:"pilot",  id:"platformLive",     label:"Travel Platform Live",                           start:"2028-06", end:"2028-06", color:"#F7B731", kind:"milestone" },
];

// ── Activities (revised May 2026 — RFI removed, governance added) ──
// Each activity: laneId = workstream, phaseId = phase window it falls in.
// Time window is derived from the phase window.
const ACTIVITIES = [
  // Phase 1: Vendor RFP & Selection (Jul–Sep 2026)
  // Combines foundational discovery (formerly Phase 1) and persona/feasibility
  // work (formerly Phase 2) — both run in parallel with the RFP process.
  ...
  { laneId:"ae",   phaseId:"phase1", label:"Map personas to pilot content and platform use cases" },
  { laneId:"ae",   phaseId:"phase1", label:"Confirm content sourcing model and editorial governance" },
  ...
  { laneId:"csa",  phaseId:"phase1", label:"Establish analytics baselines, benchmarks, and reporting logic" },
  { laneId:"csa",  phaseId:"phase1", label:"Assess revenue feasibility and partner alignment with shortlisted vendors" },
  { laneId:"lc",   phaseId:"phase1", label:"Early legal, data-rights and privacy alignment for vendor screening" },
  { laneId:"tech", phaseId:"phase1", label:"Define preliminary platform architecture and integration sequencing" },
  { laneId:"exec", phaseId:"phase1", label:"Executive alignment on finalist direction before LOI authorization" },

  // Phase 2: LOI + Contract Complete (Oct 2026 – Feb 2027)
  // Was Phase 3 in the previous plan.
  { laneId:"pmpd", phaseId:"phase2", label:"Design pilot structure, hypotheses and success criteria" },
  { laneId:"lc",   phaseId:"phase2", label:"Embed compliance, privacy and brand risk requirements into contracting" },
  { laneId:"csa",  phaseId:"phase2", label:"Finalize monetization tests and revenue success thresholds" },
  { laneId:"tech", phaseId:"phase2", label:"Plan identity resolution, data ingestion and reporting pipelines" },
  { laneId:"csa",  phaseId:"phase2", label:"Prepare internal sales enablement and operating readiness" },
  { laneId:"exec", phaseId:"phase2", label:"Approval to enter binding contractual commitments (LOI / contract execution)" },
  { laneId:"exec", phaseId:"phase2", label:"Acceptance of monetization and performance assumptions tied to investment" },

  // Phase 3: Parallel Platform Setup (Nov 2026 – Jul 2027)
  // Was Phase 4 in the previous plan.
  { laneId:"tech", phaseId:"phase3", label:"Execute platform integration and data ingestion" },
  { laneId:"ae",   phaseId:"phase3", label:"Run editorial production pilots and workflow testing" },
  { laneId:"csa",  phaseId:"phase3", label:"Implement analytics instrumentation, QA and dashboards" },
  { laneId:"tech", phaseId:"phase3", label:"Complete security, privacy and performance testing", joint:["lc"] },
  { laneId:"pmpd", phaseId:"phase3", label:"Conduct soft launch and internal beta validation" },

  // Phase 4: Pilot Execution & Learning (Aug 2027 – Apr 2028)
  // Was Phase 5 in the previous plan.
  { laneId:"pmpd", phaseId:"phase4", label:"Launch phased pilots across prioritized ZD properties" },
  { laneId:"csa",  phaseId:"phase4", label:"Test audience engagement, monetization performance and conversion" },
  { laneId:"csa",  phaseId:"phase4", label:"Ongoing performance readouts, insight synthesis and optimization loops" },
  { laneId:"pmpd", phaseId:"phase4", label:"Coordinate cross-functional iteration across editorial, commercial and technical components" },
  { laneId:"exec", phaseId:"phase4", label:"Formal go / no-go / iterate decision checkpoints", joint:["csa"] },

  // Phase 5: Travel Platform Live Decision Point (May–Jun 2028)
  // Was Phase 6 in the previous plan.
  { laneId:"csa",  phaseId:"phase5", label:"Close out pilots and consolidate cross-phase learnings" },
  { laneId:"exec", phaseId:"phase5", label:"Assess scale readiness and expansion implications" },
  { laneId:"exec", phaseId:"phase5", label:"Confirm go-forward operating and monetization strategy", joint:["csa"] },
  { laneId:"exec", phaseId:"phase5", label:"Update roadmap for post-pilot expansion or iteration", joint:["pmpd"] },
];

// ── Shared milestones (revised May 2026 — 24-month plan) ─────────
const SHARED_MILESTONES = [
  { id:"downselect",       label:"Down-Select",        date:"2026-08", code:"M1", lane:"vendor" },
  { id:"selection",        label:"Final Selection",    date:"2026-09", code:"M2", lane:"vendor" },
  { id:"loi",              label:"Letter of Intent",   date:"2026-10", code:"M3", lane:"vendor" },
  { id:"contractComplete", label:"Contract Complete",  date:"2027-02", code:"M4", lane:"vendor" },
  { id:"platformLive",     label:"Travel Platform Live", date:"2028-06", code:"P1", lane:"pilot" },
];
...
// Shared sans-serif font stack — used across all views for consistency
...
function OverviewTimeline() {
  const lanePhases = OVERVIEW_LANES.map(lane => {
    ...
    // Use the code from SHARED_MILESTONES so codes stay stable across views.
    // Fallback to M{n} if a milestone isn't in SHARED_MILESTONES.
    milestones.forEach((m, i) => {
      m.row = 0;
      const shared = SHARED_MILESTONES.find(s => s.id === m.id);
      m.code = shared ? shared.code : `M${i + 1}`;
    });
...
// ═════════════════════════════════════════════════════════════════
// SUMMARY VIEW — Matrix-based amalgamation of Overview + Phase Matrix
// Rows: Milestones, Pilots, then 6 workstream rows
// Columns: 6 phases with index + name + dates in header
// Designed for sponsor slides: sans-serif throughout, full activity
// descriptions, milestones described in full (no diamond symbols).
// ═════════════════════════════════════════════════════════════════

// Pilot metadata for the Pilots row
const SUMMARY_PILOTS = [
  { id: "p1", phaseId: "phase4", label: "Pilot 1 — RetailMeNot",                       window: "Aug–Oct 2027",     color: "#FF6B35" },
  { id: "p2", phaseId: "phase4", label: "Pilot 2 — Tech Flagships (ZDNet, PCMag, CNET)", window: "Dec 2027–Feb 2028", color: "#FF4081" },
  { id: "p3", phaseId: "phase4", label: "Pilot 3 — Mashable, Spiceworks, Lifehacker",    window: "Apr–Jun 2028",      color: "#9B59B6" },
];

function SummaryMatrix() {
  // Group activities by (laneId, phaseId)
  const cells = {};
  ...
  PHASE_WINDOWS.forEach(pw => { milestonesByPhase[pw.id] = []; });
  SHARED_MILESTONES.forEach(m => {
    // Find which phase this milestone falls within
    const phase = PHASE_WINDOWS.find(pw => {
      const mIdx = monthIndex(m.date);
      return mIdx >= monthIndex(pw.start) && mIdx <= monthIndex(pw.end);
    });
    if (phase) milestonesByPhase[phase.id].push(m);
  });

  // Group pilots by phase (all in phase4 — Pilot Execution & Learning)
  const pilotsByPhase = {};
  PHASE_WINDOWS.forEach(pw => { pilotsByPhase[pw.id] = []; });
  SUMMARY_PILOTS.forEach(p => {
    if (pilotsByPhase[p.phaseId]) pilotsByPhase[p.phaseId].push(p);
  });

  // Design tokens local to this view
  // sansFont removed — use the global SANS constant instead
  const rowBorder = t.borderSoft;
  const colBorder = t.border;
  const bgEven = t.bg;
  const bgOdd = "rgba(108,99,255,0.025)";
  const labelColBg = t.bgSoft;

  // Reusable row-header cell (left column)
  // `anchorColor` overrides the border/text accent for anchor rows (Milestones, Pilots)
  const rowHeader = (lane, subtitle, bg, anchorColor) => {
    const accent = lane ? lane.color : (anchorColor || "#F7B731");
    return (
      <div style={{
        background: bg || labelColBg,
        padding: "6px 10px",
        ...
        <div style={{
          ...
          {lane ? lane.label : (subtitle === "MILESTONES" ? "Key milestones" : "Pilot execution")}
        </div>
      </div>
    );
  };
  ...
  // Activity card
  const activityCard = (a, laneColor) => (
    <div style={{
  ...
  // Empty-cell placeholder (small centred dot)
  const emptyCell = (
    <div style={{
      display: "flex", alignItems: "center", justifyContent: "center",
      color: t.textDim, fontSize: "16px", minHeight: "28px",
    }}>·</div>
  );

  // Layout — CSS grid: narrower lane column, equal widths for 6 phase columns
  const gridCols = `140px repeat(${PHASE_WINDOWS.length}, minmax(0, 1fr))`;
  const cellPad = "6px 8px";

  // Distinct row background tints for the "anchor" rows (Milestones, Pilots)
  // vs. workstream rows, so viewers can visually separate the bands
  const milestoneRowBg = "rgba(247,183,49,0.08)";
  const pilotRowBg     = "rgba(255,107,53,0.06)";
  const workstreamRowBg = t.bg;

  const renderRow = (lane, subtitle, contentByPhase, rowBg, anchorColor) => (
    <>
      {rowHeader(lane, subtitle, rowBg, anchorColor)}
      {PHASE_WINDOWS.map(pw => {
        const content = contentByPhase[pw.id] || [];
        return (
          <div key={pw.id} style={{
            padding: cellPad,
            borderRight: `1px solid ${colBorder}`,
            ...
            {content.length === 0 ? emptyCell : content}
          </div>
        );
      })}
    </>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ padding: cellPad, borderRight: `1px solid ${colBorder}` }} />
        {PHASE_WINDOWS.map((pw, i) => (
          <div key={pw.id} style={{
            ...
            <div style={{
              ...
              color: t.textMuted,
              marginTop: "1px",
            }}>{monthName(pw.start)} – {monthName(pw.end)}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: gridCols }}>
        {renderRow(
          null,
          "MILESTONES",
          Object.fromEntries(PHASE_WINDOWS.map(pw => [
            pw.id,
            milestonesByPhase[pw.id].map(m => <div key={m.id}>{milestoneChip(m)}</div>)
          ])),
          milestoneRowBg,
          "#b88b1a"
        )}
      ...
      {/* Pilots row — with a heavier divider below to separate "anchors" band from workstreams */}
      <div style={{
        display: "grid",
        gridTemplateColumns: gridCols,
        borderBottom: `2px solid ${t.border}`,
        ...
        {renderRow(
          ...
          Object.fromEntries(PHASE_WINDOWS.map(pw => [
            pw.id,
            pilotsByPhase[pw.id].map(p => <div key={p.id}>{pilotChip(p)}</div>)
          ])),
          pilotRowBg,
          "#FF6B35"
        )}
      ...
      {WORKSTREAM_LANES.map(lane => (
        <div key={lane.id} style={{ display: "grid", gridTemplateColumns: gridCols }}>
          {renderRow(
            lane,
            null,
            Object.fromEntries(PHASE_WINDOWS.map(pw => [
              pw.id,
              cells[lane.id][pw.id].map((a, i) =>
                <div key={`${lane.id}-${pw.id}-${i}`}>{activityCard(a, lane.color)}</div>
              )
            ])),
            workstreamRowBg
          )}
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "8px",
        ...
        <span style={{ color: t.textMuted, fontFamily: SANS,
                            ...
                            <div key={i} style={{
                              ...
                              {a.joint && a.joint.length > 0 && (
                                <div style={{ marginTop: "4px", fontSize: "9px",
                                  color: t.textMuted, fontFamily: SANS,
                                  letterSpacing: "0.4px" }}>
                                  w/ {a.joint.map(j => WORKSTREAM_LANES.find(l => l.id === j)?.short).join(", ")}
                                </div>
                              )}
    ...
    <g>
      ...
      {MONTH_LABELS.map((m, i) => (
        <text key={`ml-${i}`} x={LANE_LABEL_WIDTH + i * MONTH_WIDTH + MONTH_WIDTH / 2}
          y={48} fontSize="10" fontFamily={SANS}
...
function LaneShell({ lane, laneTop, laneHeight, laneIdx, itemCount, itemLabel = "items",
  ...
  return (
    <g style={clickable ? { cursor: "pointer" } : undefined}
      ...
      <text x={18} y={laneTop + 22} fontSize="9" fontFamily={SANS}
        fill={t.label} fontWeight="600" letterSpacing="1.5">
      ...
      {clickable && (
        <g>
          ...
          <text x={LANE_LABEL_WIDTH - 20} y={laneTop + 23} fontSize="11"
            fontFamily={SANS} fill={lane.color}
      ...
      {!hideCount && (
        <text x={18} y={laneTop + laneHeight - 10} fontSize="9"
          fontFamily={SANS} fill={t.textMuted} letterSpacing="0.3">
          {itemCount} {itemLabel}
        </text>
      )}
      {children}
  ...
  if (p.kind === "milestone") {
    ...
    return (
      <g>
        ...
        <text x={cx} y={cy + 3.5} fontSize="10" fontFamily={SANS}
...
export default function RoadmapV16() {
  ...
  const views = {
    ...
    summary:    { label: "Summary",      sub: "Milestones, pilots & workstreams by phase" },
        ...
        <div style={{ position: "relative" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
            <div style={{ fontSize: "10px", letterSpacing: "4px", textTransform: "uppercase",
              color: t.accent, fontFamily: SANS }}>
            ...
            <div style={{ background: "rgba(108,99,255,0.12)",
              ...
              fontFamily: SANS, letterSpacing: "2px" }}>
              v1.9 DRAFT
            </div>
          </div>
          ...
          <p style={{ color: t.textMuted, fontSize: "13px", margin: 0,
            ...
            24-month plan with 6 workstreams and 32 activities mapped to 5 phase windows.
          </p>
        </div>
      </div>
      ...
      <div style={{ padding: "18px 40px 16px", borderBottom: `1px solid ${t.borderSoft}`,
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
          <span style={{ fontSize: "11px", letterSpacing: "1.5px",
            ...
            fontFamily: SANS, marginRight: "6px" }}>
            View:
          </span>
      ...
      <div style={{
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px",
            fontSize: "10px", fontFamily: SANS, letterSpacing: "1.5px",
          ...
          {joint.length > 0 && (
            <div style={{ marginTop: "10px", display: "flex", alignItems: "center",
              ...
              <span style={{ fontFamily: SANS, letterSpacing: "1px",
  ...
  const renderGroup = (label, items) => (
    <div>
      <div style={{ fontSize: "10px", letterSpacing: "1.5px", textTransform: "uppercase",
        color: t.textMuted, fontFamily: SANS, marginBottom: "10px" }}>
        {label}
      </div>
          ...
          <div key={m.id} style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ color: t.textMuted, fontSize: "10.5px",
                fontFamily: SANS, letterSpacing: "0.5px", marginTop: "2px" }}>
                {monthName(m.date)}
              </div>
            </div>
          </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(400px,1fr))", gap: "20px" }}>
      <div style={{ background: "rgba(247,183,49,0.07)",
        ...
        <ul style={{ margin: 0, paddingLeft: "18px", fontSize: "13px",
          ...
          <li><strong style={{ color: t.text }}>One-month gap between pilots:</strong> the visible gap between Pilot 1 and Pilot 2 (Nov 2027) and between Pilot 2 and Pilot 3 (Mar 2028) is intentional — each window is reserved for retrospective on the completed pilot and preparation for the next.</li>
      ...
      <div style={{ background: "rgba(108,99,255,0.05)",
        ...
        <ul style={{ margin: 0, paddingLeft: "18px", fontSize: "13px",
          color: t.textMuted, lineHeight: 1.7 }}>
          <li><strong style={{ color: t.text }}>RFI step removed:</strong> as of May 2026, the Vendor RFI is no longer in scope. Vendor RFP begins Jul 2026 with Down-Select retained as an early milestone within the RFP phase. Total plan duration reduced from 26 to 24 months.</li>
          <li><strong style={{ color: t.text }}>Executive governance activities:</strong> three executive-owned governance checkpoints added — finalist alignment before LOI (Phase 1), LOI/contract approval (Phase 2), and acceptance of monetization assumptions (Phase 2). Phase 3 (Platform Setup) is exception-based for executive engagement.</li>
          ...
          <li><strong style={{ color: t.text }}>Timeline compression:</strong> if dates shorten further, adjust PHASE_WINDOWS — activities rebase automatically; milestones need individual date updates.</li>
        </ul>
      </div>
    </div>
  );
