...
function SummaryMatrix() {
  ...
  const rowHeader = (lane, subtitle) => (
    <div style={{
      ...
      borderLeft: `3px solid ${lane ? lane.color : "#F7B731"}`,
      minHeight: "100%",
    }}>
      <div style={{
        fontFamily: sansFont,
  ...
  // Layout — CSS grid: 1fr for lane column, equal widths for 6 phase columns
  const gridCols = `200px repeat(${PHASE_WINDOWS.length}, minmax(0, 1fr))`;
  const cellPad = "10px 12px";

  const renderRow = (lane, subtitle, contentByPhase) => (
    <>
      {rowHeader(lane, subtitle)}
